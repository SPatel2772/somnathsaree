...
class myPDF extends FPDF{
    ...
    function CustomerName(){
        ...
        $this->Cell(37,10, 'Customer Name : ', 0,0, 'L'); 
        $this->SetFont('Arial','',12);
        $this->Cell(47.5,10, $c_name, 0,0, 'L'); 
        $this->Ln(12);
   

                    // All Records
                    $sql = "SELECT * FROM `customerlist` WHERE `customer_name` = '$c_name' ";
                    $result = mysqli_query($conn, $sql);

                    // Table Heade
                    $this->SetFont('Arial','B',15);
                    $this->Cell(34,10, 'Date', 1,0, 'C');
                    $this->Cell(58,10, 'Merchant', 1,0, 'C');
                    ...
                    while ($row = mysqli_fetch_array($result)){
                        ...
                        $this->Cell(58,7, $row['customer_merchant_name'], 1,0, 'L');
                        $this->Cell(34,7, $row['total_saree'], 1,0, 'C');
                        $this->Cell(32,7, $row['price'], 1,0, 'C');
